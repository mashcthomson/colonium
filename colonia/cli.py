...
from colonia.config import load_config, save_config
from colonia.models import CouncilJobRequest, DesktopMode, ServiceName
...
def cmd_init(_: argparse.Namespace) -> int:
    ...
    print(f"Initialized Colonia at {cfg.data_dir}")
    print(f"Config: {path}")
    print("Next:")
    print("  colonia desktop start    # open Colonia Desktop 2 (Xephyr)")
    print("  colonia browsers launch --login   # 8 Chromes with chat sites")
    return 0
...
def cmd_desktop_start(args: argparse.Namespace) -> int:
    ...
    if state.mode == DesktopMode.XEPHYR:
        print()
        print(
            "Tip: look for a window titled 'Colonia Desktop 2'. "
...
def cmd_desktop_stop(_: argparse.Namespace) -> int:
    ...
    mgr = LinuxDesktopManager()
    stopped = mgr.stop()
    print("Colonia desktop stopped." if stopped else "No Colonia desktop was running.")
    return 0
...
def cmd_browsers_launch(args: argparse.Namespace) -> int:
    ...
    if not mgr.load_state():
        print("Starting Colonia Desktop 2 first...")
        mgr.start()
    ...
    if args.login:
        print()
        print("Log into each service in Colonia Desktop 2, then run council queries.")
    return 0
...
def _print_progress_event(event: dict) -> None:
    ...
    print(
        f"[colonia] finished {finished}; pending={event.get('pending_tasks', 0)}",
        file=sys.stderr,
    )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="colonia",
        description="Colonia — multi-browser AI council on Linux Desktop 2",
    )
    sub = p.add_subparsers(dest="command", required=True)

    init_p = sub.add_parser("init", help="Create ~/.colonia config and directories")
    init_p.set_defaults(func=cmd_init)

    desk = sub.add_parser("desktop", help="Manage Colonia Desktop 2")
    desk_sub = desk.add_subparsers(dest="desktop_cmd", required=True)

    ds = desk_sub.add_parser("start", help="Start isolated desktop (Xephyr by default)")
    ds.add_argument(
        "--mode",
        choices=[m.value for m in DesktopMode],
        help="xephyr=nested window, workspace=wmctrl workspace, current=no isolation",
    )
    ds.add_argument("--display", help="Xephyr display, default :20 from config")
    ds.add_argument("--workspace", type=int, help="Workspace index for workspace mode (0-based)")
    ds.add_argument("--force", action="store_true", help="Restart Xephyr even if running")
    ds.set_defaults(func=cmd_desktop_start)

    dst = desk_sub.add_parser("stop", help="Stop Colonia Xephyr desktop")
    ...
    bl = br_sub.add_parser("launch", help="Launch Chrome instances on Desktop 2")
    ...
    bs = br_sub.add_parser("stop", help="Stop all Colonia Chrome processes")
    ...
    caps = sub.add_parser("capabilities", help="Describe Colonia as an AI-usable tool")
    ...
    ma.add_argument(
        ...
        default="claude",
        help="Service to configure. Only Claude is live-supported currently.",
    )
